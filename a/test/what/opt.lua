...
local Import = function(Asset)
	if (type(Asset) == "number") then
		...
		local Link = format("https://raw.githubusercontent.com/daximul/who/main/a/test/what/others/%s", Asset)
...
randomString = function() return GenerateGUID(HttpService, false):gsub("-", ""):sub(1, math.random(25, 30)) end
...
ChatlogAPI.gameName = tostring(MarketplaceService:GetProductInfo(game.PlaceId).Name)
...
JoinlogAPI.gameName = tostring(MarketplaceService:GetProductInfo(game.PlaceId).Name)
...
SmoothScroll = function(content, SmoothingFactor)
	...
	local syncProperty = function(prop)
		CConncet(GetPropertyChangedSignal(content, prop), function()
			if prop == "ZIndex" then
...
CConnect(Cmdbar:GetPropertyChangedSignal("Text"), function()
