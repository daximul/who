...
local _GetPlayers = GetService(game, "Players")
local _GetMarket = GetService(game, "MarketplaceService")
...
spawn(function()
	...
	Interface.MainDragFrame.Main.Pages.Server.Game.Description.DescriptionFrame.Description.Text = tostring(_GetMarket.GetProductInfo(_GetMarket, game.PlaceId).Description)
	...
	if string.len(_GetRandomMessage) >= 70 then Interface.MainDragFrame.Main.Pages.Menu.Message.Title.TextScaled = true end
...
spawn(function()
	...
	local IsA, GetChildren = game.IsA, game.GetChildren
	...
	for i,v in pairs(GetChildren(Interface.MainDragFrame.Main.Menu)) do
		if IsA(v, "TextButton") then
			CConnect(v.MouseButton1Down, function()
				...
				for i2,v2 in pairs(GetChildren(Interface.MainDragFrame.Main.Menu)) do
