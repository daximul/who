...
local createConnection = Connection.Connect
AdminConnections = {}
CConnect = function(n, f)
	local s = createConnection(n, f)
	AdminConnections[#AdminConnections + 1] = s
	return s
end
local Disconnect
do
    local CalledConnection = createConnection(Connection, function() end)
...
newCmd("killscript", {"killadmin"}, "killscript / killadmin", "Kill the Script", function(args, speaker)
	for connection in pairs(AdminConnections) do
		Disconnect(AdminConnections[connection])
	end
end)
